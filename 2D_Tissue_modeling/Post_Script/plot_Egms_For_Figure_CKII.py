...
fig = plt.figure(figsize=(8.4/1.2*1.3,6*1.25*1.))
num_row = 6;
num_col = 1;
...
data0 = np.loadtxt(folder+'BCL.500.ISO.0.1.CaMKII.CaMKII_db'+'60_0_0_.dat', unpack=True)
files = [
...
# lables = files
lables = ['No CaMKII-Cx', 'No CaMKII-K1','No CaMKII-NaV',]
...
for i in range(len(data)):
	...
	ax.plot(data0[0],data0[1]/np.max(data0[1]), 'tab:red',lw= 2., label='CaMKIIx2 + ISO')
	ax.plot(data[i][0],data[i][1]/np.max(data[i][1]), 'k',lw= 2., label=lables[i])
	ax.plot( [500,2500], [-0.9,-0.9],color='k',lw=3)
...
plt.savefig('EGms_ISO_CKII.pdf', dpi=300)
