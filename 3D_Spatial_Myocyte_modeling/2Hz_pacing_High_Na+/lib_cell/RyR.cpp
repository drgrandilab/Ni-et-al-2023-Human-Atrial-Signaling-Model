...
double RyR::Update_RyR_stochastic(double dt, double Caj, double CaSRj, double PKA_Scale, double CKII_Scale) {

	if(CKII_Scale <=1) 
		CKII_Scale = 1;
	
	double rho = rhoinf * 1.0 / (1.0 + pow((KK - (CKII_Scale-1)*200) / CaSRj, hh));
	if (rho < 0.0000001)rho = 0.0000001;
	double MM = (sqrt(1 + 8 * rho * BCSQN) - 1) / (4 * rho * BCSQN);
	
	double cp2 = Caj * Caj;
	double sgmd = cp2 * Caj / (Kcp * Kcp * Kcp + cp2 * Caj);
	double k12 = (PKA_Scale + CKII_Scale - 1) * (Ku * sgmd + pedk12);
	double k43 = (PKA_Scale + CKII_Scale - 1) * (Kb * sgmd + pedk43);
