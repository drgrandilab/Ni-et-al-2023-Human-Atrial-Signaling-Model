...
double CCell::comp_ikr(void) {
    ...
    double gkr      =(1 + para->dGkr_PKA)* 0.035 * sqrt(Ko / 5.4) * factor_rano_kr;
    // gkr = 0.035*sqrt(Ko/5.4);
    double xrss     = 1 / (1 + exp(-(y[39] + 10 + para->Vkr_PKA) / 5) );
...
double CCell::comp_iks(void) {
    ...
    double gks_junc = (1 + 1 * AF + 2 * para->kPKA_Iks) * 0.0035 * 1;
    double gks_sl   = (1 + 1 * AF + 2 * para->kPKA_Iks) * 0.0035 * 1;
    double  xsss    = 1 / (1 + exp(-(y[39] + 40 * para->kPKA_Iks + 3.8) / 14.25) );
    double tauxs    = 990.1 / (1 + exp(-(y[39] + 40 * para->kPKA_Iks + 2.436) / 14.12) );
...
double CCell::comp_ito(void) {
    double GtoFast  =  (1 + (0.6 - 1) * para->kPKA_Itof) * (1.0 - 0.7 * AF) * 0.165 * 1.0; //nS/pF
    ...
    double Vm = y[39] - para->kCKII_Itof_vshift;
    double xtoss = 1 / ( 1 + exp( -(Vm + 1) / 11 ) );
    double tauxtof = 3.5 * exp(-((Vm / 30) * (Vm / 30))) + 1.5;

    Vm = y[39];// - para.kCKII_Itof_vshift;  // no shift in ytoss
...
double CCell::comp_ikur(void) {
    ...
    double I_kur    =  (1.0 + 2 * para->kPKA_Ikur)*para->kCKII_Ikur* gkurbar * (1 * Gkur * y[58] * y[59] * (y[39] - ek));
...
double CCell::comp_ik1(void) {
    ...
    double I_ki     = (1 + (0.55 - 1) * para->kPKA_IK1) * para->kCKII_IK1_G * gkibar * ((1 + 1 * AF) * 0.0525 * sqrt(Ko / 5.4) * kiss * (y[39] - ek) );
