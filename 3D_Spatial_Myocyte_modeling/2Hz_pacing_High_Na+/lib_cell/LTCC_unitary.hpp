...
public:
	LTCC_unitary(double v) {
		// const double pca = 1.4 * 6.1 * 0.65; // original from Xianwei's code
		const double pca = 0.90*1.4 * 6.1 * 0.65; // times 0.90 to maintain total current after CaMKII effects
		z = v * F / (R * T);
		za = z * 2.0;
