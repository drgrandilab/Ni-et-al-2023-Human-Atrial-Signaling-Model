...
void CSubcell::update_LTCC_RyR(double voltage, double local_dt) {
        ...
        double NaL_ratio =  ( (14.0 / 11.0) / (1 + exp(-(para->NaV_CKp - 0.12) / 0.1)) ) * 0.999763767766896*1.02704368132844 * (1.2 * 0.003);
        INafast_gating   = (0.25 * para->kPKA_Ina_increase + 1) *Ina.Update_INa(local_dt, v, para->NaV_CKp);


        double INaL_gating = InaL.update_INaL(local_dt, v);
...
#pragma vector always
                for (int index = 0; index < num_tubule + n; index++)
        {
                if (index < num_tubule)
                        {
                                double NL  = 0;  // changed to double, because will need to compute combination;
                                int id  = Tubular_map[index];

                                // adding mode 2 here
                                int NL_m1      = LTCC_Markov_vec[id].update_states_v7(local_dt, 0.001 * cp[id], voltage);      // [A/F]
                                int NL_m2      = LTCC_Markov_M2_vec[id].update_states_v7(local_dt, 0.001 * cp[id], voltage);      // [A/F]


                                // should be a combination of NL_m1 and NL_m2
                                NL = (1 - (para->fckiim2_j+para->fpkam2)) * NL_m1 + (para->fckiim2_j+para->fpkam2) * NL_m2;

                                ica_array[id]   = para->G_LTCC * gca * ICaL_unitary.compute_LTCC_unitary(cp[id], cao) * NL;
                                ICaNa_array[id] = para->G_LTCC * update_ICaNa(nap[id], NL);      // [A]
                                ICaK_array[id]  = para->G_LTCC * update_ICaK(NL);                // [A]
                                ...
                                I_ClCa_junc_array[id]   = Fjunc * (GClCa) / (1 + (1 + (0.704 - 1) * para->kPKA_IClCa) *KdClCa / cp[id]) * (v - ecl);   // [A]

                                double ena_junc         = (1 * rtf) * log(nao / nap[id]); // [mV]
                                I_nak_junc_array[id]    = (1 * Fjunc * (IbarNaK) * fnak * Ko / (1 + std::pow((KmNaip- para->KmNaip_PKA) / (nap[id]), 4.0) ) / (Ko + KmKo) ); // [A]
                                I_nabk_junc_array[id]   = (Fjunc * (GNaB) * (v - ena_junc) ); // [A]
                                I_Na_junc_array[id]     = Fjunc * GNa_hh * (INafast_gating + NaL_ratio*INaL_gating) * (v - ena_junc); // [A]

                                I_ClCa_sl_array[id]     = tubule_flag[id] * (1 - Fjunc) * GClCa / (1 + (1 + (0.704 - 1) * para->kPKA_IClCa) *KdClCa / cs[id]) * (v - ecl);      // [A]
                                ...
                                I_nak_sl_array[id]         = tubule_flag[id] * (1 * (1 - Fjunc) * (IbarNaK) * fnak * Ko / (1 + std::pow((KmNaip- para->KmNaip_PKA) / (nas[id]), 4.0) ) / (Ko + KmKo) ); // [A]

                                double ena_sl           = (1 * rtf) * log(nao / nas[id]);         // [mV]
                                I_nabk_sl_array[id]        = tubule_flag[id] * (1 - Fjunc) * GNaB * (v - ena_sl); // [A]
                                I_Na_sl_array[id]          = tubule_flag[id] * (1 - Fjunc) * GNa_hh * (INafast_gating + NaL_ratio*INaL_gating) * (v - ena_sl); // [A]
                        ...
                        } else
                        {
                                ...
                                if (!caffeine_app)
                                {
                                        ...
                                        double RyR_koSRCa_Scale  = para->fCKII_RyR + para->fPKA_RyR - 1.0;
                                        // std::cout << "RyR_koSRCa_Scale = " << RyR_koSRCa_Scale << std::endl;
                                        PoSpark[id]  = RyR_vec[id].Update_RyR_stochastic(local_dt, cp[id], cjsr[id], para->fPKA_RyR, para->fCKII_RyR);
                                } else
                                {
                                        PoSpark[id]     = 0.2;
                                }
...
#pragma vector always
                for (int id = 0; id < nn; id++)
                {
                        ...
                        if (!caffeine_app) {
                                ...
                                double PLB_kmf_Scale = para->fCKII_PLB < para->fPKA_PLB ? para->fCKII_PLB : para->fPKA_PLB;
                                const double H  = 1.787;
                                double tmp1     = pow(ci_i / (kup *PLB_kmf_Scale), H);
                        ...
                        double ITCi     =   kon * ci_i * ( BT - cati[id] ) - para->fPKA_TnI *koff * cati[id];
                        ...
                        double Ileak    = (1.0 / 3.0 + 10 * para->RyR_CKp / 3.0) * gleak * (cjsr2 * (cnsr[id] - ci_i)) / (cjsr2 + KJSR * KJSR);
