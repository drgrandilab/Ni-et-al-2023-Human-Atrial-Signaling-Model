	...
	, cc (CCell()), m_cell_ID(cell_ID)
{
	...
	double b        = 0.6;          // [Hz]
        ...
        double nai_res  = 7.757289 + (15.13319 - 7.757289) / pow((1.0 + pow((250.0/129429500.0), 0.7283862)), 8754.445); // assign to 4 Hz Nai
				// 11.794097910198252
...
void Spatial_Cell::set_AF() {

	sc.NCX_fit_scale *= 1.4;
	cc.gkurbar *= 0.5;
	cc.gksbar *= 2.0;
	cc.gtobar *= 0.4;
}
...
void Spatial_Cell::pace_Vm_clamp(double voltage) {
        sc.update_LTCC_RyR(voltage, dt);
        sc.pace2(voltage , dt);
...
void Spatial_Cell::output_Cai(std::string folder) {
	fprintf ( output9, "%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n", 
		t, cc.v, sc.compute_ica(), sc.compute_ir(), sc.compute_serca(), 
		sc.compute_incx(), sc.compute_icabk(), sc.compute_ipca(), sc.compute_ileak(), sc.get_ina_sl(), 
		sc.get_ina_junc(), sc.get_inabk_sl(), sc.get_inabk_junc(), sc.get_inak_sl(), sc.get_inak_junc(), 
		sc.compute_ICaK(), sc.compute_ICaNa(), cc._I_to, cc._I_kr, cc._I_ks, 
		cc._I_ki, cc._I_kp, cc._I_kur, cc._I_KAch, cc._I_Clbk, 
		sc.compute_I_ClCa_sl(), sc.compute_I_ClCa_junc(), sc.get_isk_sl() + sc.get_isk_junc() 
		);
