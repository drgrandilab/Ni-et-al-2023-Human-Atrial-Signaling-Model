...
double LTCC_Markov::update_states_v9(double Caj, double Vm, double najLCC, double kjLCC, double ISO_PKA) {
	...
	// s1o = 0.0182688;   // [1/ms]
	double s1o = /*1.2 * *//*1.2**/1.2*1.4*1.3 * 5.0 * 0.0182688; // *(ISO_PKA*1+1); // [1/ms]  doubled by haibo
	// double k1o = 0.024168;    // [1/ms]
	double k1o = /*1.2 * *//*1.2**/1.2*1.4*1.3 * 5.0 * 0.024168; // *(ISO_PKA*1+1); // [1/ms]  Haibo double here...
	...
	// double sR2 = -8.5 - ISO_Shift;       // [mV] // left shift in steady-state activation
	...
	// double fcp = 0.5 / (1 + pow(cpt / Caj / 1., 1))  + 0.5 / (1 + pow(cpt / Caj, 2.0));// +0.5 / (1 + pow(cpt / Caj / 1, 3)); // + 0.01; //  Ca-dep  change hill coefficient to 4.
	...
	double Rv1_k1k2 = (0.75 +  /*0*1.0/3.0**/ISO_PKA * 0.10) / (1 + exp((Vm + 25 - 14 /*- 5*/ + 5 * fcp + ISO_Shift) / SLOPE)) + (1 - (0.75 +  /*0*1.0/3.0**/ISO_PKA * 0.10));
	...
	double Is_Vss = /*Rv1_k1k2;// */(1.0 / (1 + exp( (Vm  + 25 - 14 + 5 * fcp + ISO_Shift) / SLOPE )));
