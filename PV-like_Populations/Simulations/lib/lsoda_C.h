...
// 18:32:03, Thu, 19-September-2019, By Haibo

update with improvement in performanced tuned for electrophysiology simulations.
...
* 14:44:56, Wed, 06-February-2019, By Haibo
* Haibo updated the threshold of error for update Jacobian Matrix. Tested with the Grandi model, works fine...
* Requiring more robust test to see if this is OK...
* This speeds the code by more than 20 fold, when Vm of AP model is used as a parameter.
...
#ifndef LSODA_C_HPP
#define LSODA_C_HPP
// #include <cblas.h> // from openblas, not helpful here for small matrix though
...
public:
	...
	LSODA( int neq ): m_neq(neq)
	{
		...
		for ( int i = 1; i <= m_neq; ++i ) {
			rtol[i]     = 1.0E-5;//1.0E-10;
			atol[i]     = 1.0E-5;
		}
		...
		Error_Threshold_Jac = 5;//*5; // error threshold for updating Jacobian matrix. original value = 1.0h// change to 5 when parameters are constantly being updated to
		// reduce needs for updating Jacobian (e.g., in tissue simulations
		// where Vm is taken out of the ODE and thus used as a parameter. Please use with care)
		// 16:17:47, Wed, 06-February-2019, By Haibo

		HMAX_USER = 2; // maximum time step for the code integration hmax
		local_time = 0.0;
	...
	void dscal(  const int&  n, const  double & da, double *dx)
	{
	 ...
	 */
	// double ddot_new( int N, double *a, int inca, double *b, int incb ) {
	...
	double ddot( int n, double *dx, int incx, double *dy, int incy )
	{
		...
		if (n <= 0)
			return 0.0;
		// return ddot_( n, dx, 1, dy, 1);// *a, int inca, double *b, int incb )
		// double res= ddot_new(n, dx+1, incx, dy+1, incy);
		// return res;
	...
	double ddot( int nn, double *dx, double *dy)
	{
	...
	// void ddot_kernel_5(int n, double *x, double *y, double *d)
	// {
	//     int i = 0;
	//     double dot = 0.0;
	...
	void daxpy( const int & n, const double&  da, double *dx, double *dy ) //***changed double *da to double da
	{
	...
	void dgesl( double **a, int n, int *ipvt, double *b, int job )
	{
		...
		if (job == 0) {
			 ...
			 */
			for ( k = 1; k <= n; k++) {
				// t_dot = cblas_ddot(k - 1, a[k]+1, 1, b+1, 1);
				t_dot = ddot(k - 1, a[k], b);
				b[k] = (b[k] - t_dot) / a[k][k];
			}
			/*
			 Now solve U * x = y.
			 */
			for ( k = n - 1; k >= 1; k--) {
				// b[k] = b[k] + cblas_ddot(n - k, a[k] + k+1, 1, b + k+1, 1);
				b[k] = b[k] + ddot(n - k, a[k] + k, b + k);
				int j = ipvt[k];
				if (j != k) {
					t_dot = b[j];
					b[j] = b[k];
		...
		for ( k = 1; k <= n - 1; k++) {
			...
			// cblas_daxpy(n - k, t_dot, a[k] + k+1, 1, b + k+1, 1);
			daxpy(n - k, t_dot, a[k] + k, b + k);
		}
		/*`
		...
		for ( k = n; k >= 1; k--) {
			b[k] = b[k] / a[k][k];
			t_dot = -b[k];
			// cblas_daxpy(k - 1, t_dot, a[k]+1, 1, b+1, 1);
			daxpy(k - 1, t_dot, a[k], b);
		}
	}
	...
	void dgefa( double **a, int n, int *ipvt, int *info )
	{
		...
		*info = 0;
		for (int k = 1; k <= n - 1; k++) {
			 ...
			 */
			// int j = cblas_idamax(n - k + 1, a[k] + k - 1, 1) + k - 1;
			int j = idamax(n - k + 1, pointer + k - 1) + k - 1;
			ipvt[k] = j;
			/*
			 ...
			 */
			temp = -1. / pointer[k];
			// dscal(n - k, temp, pointer + k, 1);
			dscal(n - k, temp, pointer + k);
			/*
			...
			for (int i = k + 1; i <= n; i++) {
				...
				// daxpy(n - k, temp, pointer + k, 1, a[i] + k, 1);
				daxpy(n - k, temp, pointer + k, a[i] + k);
			}
		}           /* end k-loop  */
	...
	void lsoda(void (*f)(double, double *, double *, void *), double *y, double *t, double tout, void *_data)
	{
		...
		if (istate == 1) {
			...
			for (int i = 0; i <= lenyh; i++) {
				yh[i] = new double [1 + nyh];
				for (int j = 0; j < 1 + nyh; ++j)
				{
					yh[i][j] = 0;
				}
			}
	...
	void prja( int m_neq, double *y, void (*f)(double, double *, double *, void *), void *_data )
	{
		...
		if (miter == 2) {
			...
			double r0 = 1000. * fabs(h) * ETA * ((double) n) * fac;
			if (r0 == 0.)
				r0 = 1.;
			for (int j = 1; j <= n; j++) {
				double yj = y[j];
				double r = std::max(sqrteta * fabs(yj), r0 / ewt[j]);
				y[j] += r;
				fac = -hl0 / r;
	 ...
	 */
	double vmnorm( const int n, double *v, double *w )
	{
		// int             i;
		double          vm = 0.;
	 ...
	 */
	double fnorm( const int n, double **a, double *w )
	{
		// int             i, j;
		double          an = 0; //, *ap1;
		// double  sum;
...
private:
	...
	double Error_Threshold_Jac, HMAX_USER;
	// ODE *ode;
	int m_neq;
