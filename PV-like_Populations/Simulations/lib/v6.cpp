...
double LTCC_Markov::update_states_v6(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	//  Voltage- and Ca-dependent Parameters
	 // double fcp= 1 / (1 + pow(cpt / Caj / aff / 3, 2));        //  Ca-dep
	...
	// k3 = 0.05* betaLCC;
	// k3 = betaLCC*0.01;  // this accerates recovery...
	//  k4
	...
	double Tv1_K5  =  45 + 60*exp(-(Vm+50)*(Vm+50)/150) -20* 1.0 / (1 + exp((Vm+15)/-3));// 0.2 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.


	// if(Vm>-20)
	// 	Tv1_K5=20;//
	double fcp_k5 =  fcp;//0.99 / (1 + pow(cpt / Caj / aff, 4)) + 0.01;// + 0.49 / (1 + pow(0.0005 / Caj, 4)) + 0.01;
	double Is_Vss = (1.0 / (1 + exp( (Vm  + 30.5 + -10 + 3 * fcp) / 6.45 )));
	k5     =  Is_Vss / Tv1_K5 / (fcp_k5 * 1.0 + 1.0);
	k6      = (1 - Is_Vss) / Tv1_K5 * (fcp_k5 * 1.0 + 1.0);


	Is_Vss = (1.0 / (1 + exp( (Vm  + 30.5 + -10 + 3 * fcp) / 8 )));
	double Tv1_K9  = 50 + 20*exp(-(Vm+40)*(Vm+40)/150);
	...
	*/
	...
	double Tv1  =  50 + 4000*exp(-(Vm+50)*(Vm+50)/150) + 120* 1.0 / (1 + exp((Vm+40)/-3));//1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	double SLOPE = 8;
	double Is_Vss_V = (1 / (1 + exp( (Vm + 30.5) / SLOPE )) + 0);

	//  k5p=ICa_speed*(1-Rv2)/Tv2;

	//  k5p=ICa_speed*(1-Rv2)/Tv1;
	double Tv1_K5p =  50 + 4000*exp(-(Vm+50)*(Vm+50)/150)+ 120* 1.0 / (1 + exp((Vm+40)/-3));//1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82 * 0.5)))) + 60; ///*+ 60 / (1 + exp((Vm + 70) / (2.0)))*/; //  haibo changed here.
	...
	Rv1_k1k2 = 1 / (1 + exp((Vm + 27 - 4) / SLOPE));
	...
	double fcp_k15 = 0.4 / (1 + pow(cpt / Caj / aff / 3, 2))  + 0.6 / (1 + pow(cpt / Caj / aff, 4)) ;
...
double LTCC_Markov::update_states_v6_mode_2(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	*/
	...
	// Is_Vss_V = (1 / (1 + exp( (Vm + 30.5) / SLOPE )) +0);
	s1 = k1;
	s2 = k2;
