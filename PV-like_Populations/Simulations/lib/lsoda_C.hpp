...
public:
	...
	LSODA( int neq ): m_neq(neq)
	{
		...
		Error_Threshold_Jac = 1.0;//*5; // error threshold for updating Jacobian matrix. original value = 1.0h// change to 5 when parameters are constantly being updated to
...
// #pragma ivdep
	...
	void ddot_kernel_8(int n, double *x, double *y, double *d)
	{
		int i = 0;
		double dot = 0.0;
	...
	double dot_compute(int n, double *x, int inc_x, double *y, int inc_y)
	{
		int i = 0;
		int ix = 0, iy = 0;
		...
		*info = 0;
		for (k = 1; k <= n - 1; k++) {
			...
			for (i = k + 1; i <= n; i++) {
				...
				daxpy(n - k, temp, a[k] + k, 1, a[i] + k, 1);
			}
		}           /* end k-loop  */
