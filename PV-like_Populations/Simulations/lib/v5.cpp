...
double LTCC_Markov::update_states_v5(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	// s1o = 0.0182688;   // [1/ms]
	double s1o = /*1.2 * */4.1 * 0.0182688; // [1/ms]  doubled by haibo
	// double k1o = 0.024168;    // [1/ms]
	double k1o = /*1.2 * */4.1 * 0.024168; // [1/ms]  Haibo double here...
	...
	double fcp = 0.5 / (1 + pow(cpt / Caj / aff / 3, 2))  + 0.5 / (1 + pow(cpt / Caj / aff, 4)) ; // + 0.01; //  Ca-dep  change hill coefficient to 4.
	...
	double Rv1 = aRv1 / (1 + exp(-(Vm - sRv1) * pRv1)) + 0.15;
	//  Tv1=aTv1/(1+exp(-(Vm-sTv1)*pTv1));

	double Tv1_K5  = 45 + 1000*exp(-(Vm+55)*(Vm+55)/50);// 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 50;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	double Rv1_k1k2 = 0.9 / (1 + exp((Vm + 27 - 14 + 3 * fcp) / 6)) + 0.10;
	...
	*/
	...
	k7 = flag_7_state * ICa_speed * (1 - Rv1_k1k2) / 55.0; // tau_k1k2;
	k8 = flag_7_state * ICa_speed * Rv1_k1k2 / 55.0; //  tau_k1k2 / (1);
	//  k8=k2;
	//  k7=k1;
...
double LTCC_Markov::update_states_v5_mode_2(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	double Tv1_K5  = 45 + 1000*exp(-(Vm+40)*(Vm+40)/50);// 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 50;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
