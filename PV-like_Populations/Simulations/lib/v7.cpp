...
double LTCC_Markov::update_states_v7(double Caj, double Vm, double najLCC, double kjLCC, double ISO_PKA) {
	...
	double cpt = 3.75e-3 /** 0.5 */ /** 0.55*/ * (1 - ISO_PKA * 0.4) ; // [mM]
	double cat = 7.617e-3;    // [mM]
	// s1o = 0.0182688;   // [1/ms]
	double s1o = /*1.2 * */1.0*4.2 * 0.0182688;// *(ISO_PKA*1+1); // [1/ms]  doubled by haibo
	// double k1o = 0.024168;    // [1/ms]
	double k1o = /*1.2 * */1.0*4.2 * 0.024168;// *(ISO_PKA*1+1); // [1/ms]  Haibo double here...
	...
	double fcp = 0.5 / (1 + pow(cpt / Caj / aff / 1, 1))  + 0.5 / (1 + pow(cpt / Caj / aff, 1)) ; // + 0.01; //  Ca-dep  change hill coefficient to 4.
	...
	double tau_k1k2 = 1.0 / (k1o * fcp /*+ 0.01*/) /*+  (1.0 + 200.0 / (1 +exp((Vm+40)/2)))*/;// )*(1.0 + 5.0 / (1 +exp((Vm+40)/3)));
	double Rv1_k1k2 = (0.85 +  ISO_PKA * 0.15) / (1 + exp((Vm + 30 - 14 + 3 * fcp + ISO_Shift) / SLOPE)) + (1 - (0.85 +  ISO_PKA * 0.15));
	// Rv1_k1k2 = 0.85 / (1 + exp((Vm + 30 - 14 + 3 * fcp + ISO_Shift) / SLOPE)) + 0.15;
	...
	double Is_Vss = (1.0 / (1 + exp( (Vm  + 30.0 + -14 + 3 * fcp + ISO_Shift) / SLOPE )));
	...
	*/
	...
	double Tv1  =  45 + 3000 * exp(-(Vm + 50 + ISO_Shift) * (Vm + 50 + ISO_Shift) / 150) + 200 * 1.0 / (1 + exp((Vm + 60 + ISO_Shift) / -3)); //1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	double Tv1_K5p =  45 + 2000 * exp(-(Vm + 50 + ISO_Shift) * (Vm + 50 + ISO_Shift) / 150) + 200 * 1.0 / (1 + exp((Vm + 60 + ISO_Shift) / -3)); //1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82 * 0.5)))) + 60; ///*+ 60 / (1 + exp((Vm + 70) / (2.0)))*/; //  haibo changed here.
	...
	Rv1_k1k2 = 1 / (1 + exp((Vm + 30 - 14 + 8 * fcp + ISO_Shift) / SLOPE));
	//  Rv1_k1k2 =(1 / (1 + 0)) * (0 + Rv1_k1k2);
	k7 = flag_7_state * ICa_speed * (1 - Rv1_k1k2) / 50; // tau_k1k2;
	k8 = flag_7_state * ICa_speed * Rv1_k1k2 / 50; //  tau_k1k2 / (1);
	//  k8=k2;
	//  k7=k1;

	double Tv1_K9  = 60 + 30 * exp(-(Vm + 55 + ISO_Shift) * (Vm + 55 + ISO_Shift) / 150);
	Is_Vss = Rv1_k1k2;
	...
	double fcp_k15 = fcp;//0.5 / (1 + pow(cpt / Caj / aff / 2.5, 2))  + 0.5 / (1 + pow(cpt / Caj / aff, 1)) ;
	double k15_factor = 1;
	double tau_k15 = 1 / (0.5 * fcp_k15 + 0.01) ; //*(1.0 + 100* 1.0/(1+exp((Vm+40)/-5))) /*+ 2.0*/; // tau for k15 k16
	double k15 = k15_factor * (1 - fcp_k15) / tau_k15;
	double k15p = k15_factor * fcp_k15 / tau_k15;
	double tau_k16 = 1 / (0.5 * fcp_k15 + 0.01) ; //*(1.0 + 100* 1.0/(1+exp((Vm+40)/-5))) /*+ 2.0*/; // tau for k15 k16
...
double LTCC_Markov::update_states_v7_mode_2(double Caj, double Vm, double najLCC, double kjLCC, double ISO_PKA) {
	double ISO_Shift = ISO_PKA * 3;
	...
	double cat = 7.617e-3;    // [mM]
	// s1o = 0.0182688;   // [1/ms]
	double s1o = /*1.2 * */1.0*4.2 * 0.018268;//8*(ISO_PKA*1+1) ; // [1/ms]  doubled by haibo
	// double k1o = 0.024168;    // [1/ms]
	double k1o = /*1.2 * */1.0*4.2 * 0.024168;// *(ISO_PKA*1+1); // [1/ms]  Haibo double here...
	// double k1o = s1o;///*1.2 * */3.0 * 0.024168; // [1/ms]  Haibo double here...
	double k2o = 0.000103615; // [1/ms]
	...
	//  aff = 2;
	//  fcp=0.5/(1+(cpt/Caj/aff)^2) +0.49/(1+(cpt/Caj/aff)^4)+0.01;        //  Ca-dep  change hill coefficient to 2.
	double fcp = 0.5 / (1 + pow(cpt / Caj / aff / 1, 1))  + 0.50 / (1 + pow(cpt / Caj / aff, 1)) ; // + 0.01; //  Ca-dep  change hill coefficient to 4.
	//  fcp=0.99/(1+(cpt/Caj/aff)^2) +0.01;        //  Ca-dep  change hill coefficient to 2.
	...
	// r1 = ICa_speed * 0.5 / 1.667;
	// r2 = ICa_speed * (1 - 0.5) / 1.667;//  * (1.0 + 1.5 / (1 + exp((Vm + 50) / 1)));
	...
	//  k1 = flag_7_state*ICa_speed*k1o*fcp;
	double tau_k1k2 = 1.0 / (k1o * fcp /*+ 0.01*/) /*+  (1.0 + 200.0 / (1 +exp((Vm+40)/2)))*/;// )*(1.0 + 5.0 / (1 +exp((Vm+40)/3)));
	// double Rv1_k1k2 = 0.85 / (1 + exp((Vm + 30 - 14 + 3 * fcp + ISO_Shift) / SLOPE)) + 0.15;
	double Rv1_k1k2 = (0.85 +  ISO_PKA * 0.15) / (1 + exp((Vm + 30 - 14 + 3 * fcp + ISO_Shift) / SLOPE)) + (1 - (0.85 +  ISO_PKA * 0.15));
	// Rv1_k1k2 = 0.85 / (1 + exp((Vm + 30 - 14 + 3 * fcp + ISO_Shift) / SLOPE)) + 0.15;
	// Rv1_k1k2 =(1 / (1 + 0.9)) * (0.1 + Rv1_k1k2);
	k1 = flag_7_state * ICa_speed * (1 - Rv1_k1k2) / tau_k1k2;
