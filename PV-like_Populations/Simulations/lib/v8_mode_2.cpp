...
double LTCC_Markov::update_states_v8_mode_2(double Caj, double Vm, double najLCC, double kjLCC, double ISO_PKA) {
	...
	double cpt = 3.75e-3 /** 0.9*/; // 3.75e-3 /** 0.5 */ /** 0.55*/ * (1 - ISO_PKA * 0.4) ; // [mM]
	double cat = 7.617e-3;    // [mM]
	// s1o = 0.0182688;   // [1/ms]
	double s1o = /*1.2 * */1.4*1.3 * 5.0 * 0.0182688; // *(ISO_PKA*1+1); // [1/ms]  doubled by haibo
	// double k1o = 0.024168;    // [1/ms]
	double k1o = /*1.2 * */1.4*1.3 * 5.0 * 0.024168; // *(ISO_PKA*1+1); // [1/ms]  Haibo double here...
	...
	double sR2 = -9.5 - ISO_Shift;       // [mV] // left shift in steady-state activation
	double pR2 = 1.0 / 6.5;//6.5; //0.145;       // [1/mV]  // was 6.0
	double aT2 = 1;           // [1/ms]
	double sT2 = -1000;       // [mV]
	...
	// double fcp = 0.2 / (1 + pow(cpt / Caj / aff / 2, 2))  + 0.3 1 / (1 + pow(cpt / Caj, 1.0)) ; // + 0.01; //  Ca-dep  change hill coefficient to 4.
	double fcp = 0.2 / (1 + pow(cpt / Caj, 1))  + 0.3 / (1 + pow(cpt / Caj, 2.0)) +0.5 / (1 + pow(cpt / Caj, 3)); // + 0.01; //  Ca-dep  change hill coefficient to 4.
	// double fcp = 0.4 / (1 + pow(cpt / Caj / 1., 1))  + 0.6 / (1 + pow(cpt / Caj, 2.0));// +0.5 / (1 + pow(cpt / Caj / 1, 3)); // + 0.01; //  Ca-dep  change hill coefficient to 4.
	...
	double tau_k1k2 = 1.0 / (k1o * fcp );// +  (/*1 + */100.0 / (1 + exp((Vm + 40) / 2))); // )*(1.0 + 5.0 / (1 +exp((Vm+40)/3)));

	// tau_k1k2 = 1.0 / k1o + (200/(  1+exp((fcp-0.4)/0.08)));//1.0 / k1o / (1/(  1+exp((fcp-0.9)/-0.3)))/*1.0 / k1o / (1.0/(  1+exp((fcp-0.8)/-0.2)))*/+  (/*1 + */200.0 / (1 + exp((Vm + 40) / 2)));
	// tau_k1k2 = (1.0 / (k1o*fcp) + (200/(  1+exp((fcp-0.4)/0.08)))) * 0.5;//1.0 / k1o / (1/(  1+exp((fcp-0.9)/-0.3)))/*1.0 / k1o / (1.0/(  1+exp((fcp-0.8)/-0.2)))*/+  (/*1 + */200.0 / (1 + exp((Vm + 40) / 2)));

	// double tau_k1k2 = 5*1.0 / (k1o ) /(1 + (fcp/0.5))+  (/*1 + */200.0 / (1 + exp((Vm + 40) / 2))); 
	// double tau_k1k2 = 5*1.0 / (k1o ) + 100* (1-fcp)+  (/*1 + */200.0 / (1 + exp((Vm + 40) / 2))); 
	// double Rv1_k1k2 = (0.85 +  ISO_PKA * 0.15) / (1 + exp((Vm + 27 - 14 /*- 5*/ + 5 * fcp + ISO_Shift) / SLOPE)) + (1 - (0.85 +  ISO_PKA * 0.15));
	double Rv1_k1k2 = (0.8 +  ISO_PKA * 0.15) / (1 + exp((Vm + 27 - 14 /*- 5*/ + 5 * fcp + ISO_Shift) / SLOPE)) + (1 - (0.8 +  ISO_PKA * 0.15));
	// Rv1_k1k2 = 0.85 / (1 + exp((Vm + 30 - 14 + 5 * fcp + ISO_Shift) / SLOPE)) + 0.15;
	...
	// double Tv1_K5  = (20 + 30 * exp(-(Vm + 50 + ISO_Shift) * (Vm + 50 + ISO_Shift) / 150) +   30.0 / (1 + exp((Vm + 25 + ISO_Shift) / 5))) ; // /*+ 30 * exp(-(Vm + 60) * (Vm + 60) / 5);*/; // 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 50;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	double Tv1_K5  = 20 + (1 - fcp) * 100 / (1 + exp((Vm + 40 + ISO_Shift) / -5))
	                 +  (0.8 + 3 * fcp) * ( 150 * exp(-(Vm + 45 + ISO_Shift) * (Vm + 45 + ISO_Shift) / 50) + 30.0 / (1 + exp((Vm + 40 + ISO_Shift) / 5))) ; // /*+ 30 * exp(-(Vm + 60) * (Vm + 60) / 5);*/; // 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 50;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	double fcp_k5 =  fcp;//0.99 / (1 + pow(cpt / Caj / aff, 1)) + 0.01;// + 0.49 / (1 + pow(0.0005 / Caj, 4)) + 0.01;
	double Is_Vss = Rv1_k1k2;// (1.0 / (1 + exp( (Vm  + 30 - 14 + 5 * fcp + ISO_Shift) / SLOPE )));
	...
	*/
	...
	double Tv1  =  56 + 3000 * exp(-(Vm + 50 + ISO_Shift) * (Vm + 50 + ISO_Shift) / 150) + 200 * 1.0 / (1 + exp((Vm + 60 + ISO_Shift) / -3)); //1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.


	double Is_Vss_V = (1 / (1 + exp( (Vm + 27.0) / SLOPE )) + 0);
	...
	// double Tv1_K5p =  56 + 1700 * exp(-(Vm + 42 + ISO_Shift) * (Vm + 42 + ISO_Shift) / 150) + 200 * 1.0 / (1 + exp((Vm + 60 + ISO_Shift) / -3)); //1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82 * 0.5)))) + 60; ///*+ 60 / (1 + exp((Vm + 70) / (2.0)))*/; //  haibo changed here.
	...
	s1p = k1p;//ICa_speed * 1 / (Trev1 * (1 + Frev1));
	//  s2p=Frev1*s1p;                            //  REV
	...
	s1 = flag_7_state * ICa_speed * k1o * fcp ;///** (1.0 + 2.0 / (1 +exp((Vm-20 )/-5.0)))*/ / (1.0 + 200.0 / (1 + exp((Vm + 60) / 3)));
	...
	double fcp_k15 = fcp;//1 / (1 + pow(0.005 / Caj , 1)) ;//0.5 / (1 + pow(cpt / Caj / aff / 2.5, 2))  + 0.5 / (1 + pow(cpt / Caj / aff, 1)) ;
	double k15_factor = 1;
	double tau_k15 = 150;//1 / (0.5 * fcp_k15 + 0.01) ; //*(1.0 + 100* 1.0/(1+exp((Vm+40)/-5))) /*+ 2.0*/; // tau for k15 k16
	double k15 = k15_factor * (1 - fcp_k15) / tau_k15;
	double k15p = k15_factor * fcp_k15 / tau_k15;
	double tau_k16 = 150;//1 / (0.5 * fcp_k15 + 0.01) ; //*(1.0 + 100* 1.0/(1+exp((Vm+40)/-5))) /*+ 2.0*/; // tau for k15 k16

	double k16 = k15;//0*k15_factor*0.01;//k15_factor * (1 - fcp_k15) / tau_k16;
	double k16p = k15p;//0*k15_factor * fcp_k15 / tau_k16;
