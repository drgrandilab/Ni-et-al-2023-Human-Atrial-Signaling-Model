...
// this addes more states to the LTCC model
double LTCC_Markov::update_states_v4(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	double Tv1  = 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	double Is_Vss = (1.0 / (1 + exp( (Vm  + 27.5 + -14 + 3 * fcp) / 6 )));
	k5     =  Is_Vss / Tv1 / (fcp_k5 * 1.0 + 1.0);
	k6      = (1 - Is_Vss) / Tv1 * (fcp_k5 * 1.0 + 1.0) * 1;
	...
	*/
	...
	Tv1  = 0.5 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	double Tv1_K5p = 0.5 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60; ///*+ 60 / (1 + exp((Vm + 70) / (2.0)))*/; //  haibo changed here.
...
double LTCC_Markov::update_states_v4_plus(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	// sR2 = -2;          // [mV]
	double sR2 = -9;          // [mV] // left shift in steady-state activation
	double pR2 = 1.0 / 6.0;//6.5; //0.145;       // [1/mV]  // was 6.0
	double aT2 = 1;           // [1/ms]
	double sT2 = -1000;       // [mV]
	double pT2 = 0.100;       // [1/mV]
	double aR1 = 0.09091;
	...
	double Tv1_K5  = 60 + 20*exp(-(Vm+50)*(Vm+50)/150);// 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 50;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	//  k1 = flag_7_state*ICa_speed*k1o*fcp;
	double tau_k1k2 = 1.0 / (k1o * fcp /*+ 0.01*/); //  * (1.0 + 2.0 / (1 +exp((Vm-20)/-5))))*(1.0 + 5.0 / (1 +exp((Vm+40)/3)));
	double Rv1_k1k2 = 0.8 / (1 + exp((Vm + 27 - 14 + 3 * fcp) / 6)) + 0.2;
	// Rv1_k1k2 =(1 / (1 + 0.9)) * (0.1 + Rv1_k1k2);
	k1 = flag_7_state * ICa_speed * (1 - Rv1_k1k2) / tau_k1k2;
	k2 = flag_7_state * ICa_speed * Rv1_k1k2  / tau_k1k2 ;
	//  k2=ICa_speed*k2o;
	...
	// double fcp_k5 = fcp;// Caj / (Caj + 0.01);
	double fcp_k5 =  fcp;//0.99 / (1 + pow(cpt / Caj / aff, 4)) + 0.01;// + 0.49 / (1 + pow(0.0005 / Caj, 4)) + 0.01;
	double Is_Vss = (1.0 / (1 + exp( (Vm  + 27.5 + -14 + 3 * fcp) / 6.45 )));
	...
	*/
	...
	double Tv1  =  60 + 4000*exp(-(Vm+50)*(Vm+50)/150);//1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 60;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.

	k1p = ICa_speed * Rv1 / Tv1;
	k2p = 0.2 * ICa_speed * (1 - Rv1) / Tv1;
	k3p = ICa_speed * 1 / (Trev2 * (1 + Frev2));
	double SLOPE = 8;
	double Is_Vss_V = (1 / (1 + exp( (Vm + 27.5) / SLOPE )) + 0);

	//  k5p=ICa_speed*(1-Rv2)/Tv2;

	//  k5p=ICa_speed*(1-Rv2)/Tv1;
	double Tv1_K5p =  60 + 4000*exp(-(Vm+50)*(Vm+50)/150);//1 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82 * 0.5)))) + 60; ///*+ 60 / (1 + exp((Vm + 70) / (2.0)))*/; //  haibo changed here.
	k5p = Is_Vss_V / Tv1_K5p ;
	k6p =  (1 - Is_Vss_V) / Tv1_K5p;
	//  k6p=ICa_speed*Rv2/Tv2;
	//  k6p=ICa_speed*Rv2/Tv1;
	...
	// Is_Vss_V = (1 / (1 + exp( (Vm + 27.5) / SLOPE )) +0);
	s1 = k1;
	s2 = k2;
	...
	double Tv1_K9  = 60 + 50*exp(-(Vm+50)*(Vm+50)/150);
...
double LTCC_Markov::update_states_v4_plus_mode_2(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	// sR2 = -2;          // [mV]
	double sR2 = -9;          // [mV] // left shift in steady-state activation
	double pR2 = 1.0 /6.0;//s 6.5; //0.145;       // [1/mV]  // was 6.0
	double aT2 = 1;           // [1/ms]
	double sT2 = -1000;       // [mV]
	double pT2 = 0.100;       // [1/mV]
	double aR1 = 0.5;//0.09091;  0.5 for m2
	double sR1 = -1000;       // [mV]
	double pR1 = 0.100;       // [1/mV]
	double aT1 = 1.667;// 0.30303;     // [1/ms] 1.667 for m2
	double sT1 = -1000;       // [mV]
	double pT1 = 0.100;       // [1/mV]
	double aRv2 = 0.9;
	double sRv2 = -29;        // [mV]
	...
	*/
	...
	double k13 = 0.09091/0.30303;//0.09/0.3 /** 0.1*/;
	double k14 =  k13 * k2 * r1 * s1 / ( s2 * r2 * k1);
...
double LTCC_Markov::update_states_v4_plus2(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	double Tv1_K5  = 60 + 60*exp(-(Vm+50)*(Vm+50)/150);// 0.05 * 8.91 * (49.008 / (1 + exp((Vm + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((Vm + 0.6476 + 64) / (-4.69634 * 0.82)))) + 50;// + 60 / (1 + exp((Vm + 70) / (2.0))); //  haibo changed here.
	...
	*/
	...
	Rv1_k1k2 = 1 / (1 + exp((Vm + 27 - 12 + 8 * fcp) / SLOPE));
	//  Rv1_k1k2 =(1 / (1 + 0)) * (0 + Rv1_k1k2);
	double tau_k78 = 30 + 30.0 / (1.0 + exp( (Vm + 30)/-3 ));
	k7 = flag_7_state * ICa_speed * (1 - Rv1_k1k2) / tau_k78; // tau_k1k2;
	k8 = flag_7_state * ICa_speed * Rv1_k1k2 / tau_k78; //  tau_k1k2 / (1);
	//  k8=k2;
	//  k7=k1;


	double Tv1_K9  = 60 + 40*exp(-(Vm+50)*(Vm+50)/150);

	 Is_Vss = (1.0 / (1 + exp( (Vm   + 27.5 + -14 + 3.0 * fcp) / 6 )));

	k9 = 6 * Is_Vss / Tv1_K9;
	k10 = 6 * (1 - Is_Vss) / Tv1_K9/(1+3.5*fcp);
...
double LTCC_Markov::update_states_v4_plus2_mode_2(double Caj, double Vm, double najLCC, double kjLCC) {
	int flag_7_state = 1;
	...
	double sT2 = -1000;       // [mV]
	double pT2 = 0.100;       // [1/mV]
	double aR1 = 0.5;//0.09091;
	double sR1 = -1000;       // [mV]
	double pR1 = 0.100;       // [1/mV]
	double aT1 = 1.6667;//0.30303;     // [1/ms]
	double sT1 = -1000;       // [mV]
	double pT1 = 0.100;       // [1/mV]
	double aRv2 = 0.9;
	double sRv2 = -29;        // [mV]
...
// 	// sR2 = -2;          // [mV]
// 	double sR2 = -9;          // [mV] // left shift in steady-state activation
// 	double pR2 = 1.0 / 6.3; //0.145;       // [1/mV]  // was 6.0
// 	double aT2 = 1;           // [1/ms]
// 	double sT2 = -1000;       // [mV]
// 	double pT2 = 0.100;       // [1/mV]
// 	double aR1 = 0.09091;
...
// 	double Rv1_k1k2 = 0.85 / (1 + exp((Vm + 27 - 14 + 3 * fcp) / 6)) + 0.15;
// 	// Rv1_k1k2 =(1 / (1 + 0.9)) * (0.1 + Rv1_k1k2);
// 	k1 = flag_7_state * ICa_speed * (1 - Rv1_k1k2) / tau_k1k2;
// 	k2 = flag_7_state * ICa_speed * Rv1_k1k2  / tau_k1k2 ;
// 	//  k2=ICa_speed*k2o;
