...
int main(int argc, char const *argv[])
{
	...
	if (argc == 21 + 5 + 1) {
		for (int i = 0; i < 21; ++i)
		{
			scale[i] = atof(argv[i + 5 + 1]);
			std::cerr << scale[i] << std::endl;
		}
		Atrial_wraper.cell.cell_para.set_scaling_for_phos_levels(scale);
	}
	...
	while (outputTime < Total_time) {
		...
		*/
		...
		// if (count % outfreq == 0 and outputTime >=  0 * Total_time - 30e3 ) // 0*370e3 - 10)  // 1 Hz
		if (count % outfreq == 0 and outputTime >=  Total_time - 100e3 ) // 0*370e3 - 10) // 2 Hz
		{
