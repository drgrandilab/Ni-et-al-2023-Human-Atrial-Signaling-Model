...
% parameter_names = [{'INa'},{'INaL'},{'INab'},{'INaK'},{'Ito'},{'IKur'},{'IK2p'},{'IKr'},{'IKs'},{'IK1'},{'IKp'}, {'IKCa'},{'ICaL'},{'ICab'},{'ICap'},{'INCX'},{'IClCa'},{'IClb'},{'RyR rel'},{'JSerca'},{'RyR leak'},{'Ca Buffer-Cleft'},{'Ca Buffer-Cyt'},{'Ca Buffer-SR'}, {'Interaction'}]
...
% x = reshape(load('1Hz_ss_sim/BCL.1000.ISO.0.1.CaMKII_inhb.0.CaMKII_db.1_EAD_DAD_count.csv'), 7, [])';
x = reshape(load('SS_new/BCL.1000.ISO.0.1.CaMKII_inhb.0.CaMKII_db.0_EAD_DAD_count.csv'), 109, [])';
% x = reshape(load('1Hz_full_sim/1Hz_ISO_CKII_db_EAD_DAD_count.csv'), 7, [])';
...
figname = '1Hz_ISO_CKII_normal_EAD_count.eps';
arrhythmia_presence =  (logistic_EAD>1/2);

disp('filename');
disp(figname);

% remove cells with persistent DADs
logistic_DAD_multi = (y(:,6)>=10);

index = (logistic_DAD_multi < 0.5);
% figname = '1Hz_ISO_DAD_count.eps';
% arrhythmia_presence =  (logistic_DAD>1/2);


arrhythmia_presence = arrhythmia_presence (index);
rm_IKACH_parameters = rm_IKACH_parameters (index,:);
...
FP = (arrhythmia_presence < 0.5 & P_arrhythmia_array' > 0.5);
FN = (arrhythmia_presence > 0.5 & P_arrhythmia_array' < 0.5);



figure(5),
boxplot(rm_IKACH_parameters(FP,:)); 
...
figure(6),
boxplot(rm_IKACH_parameters(FN,:)); 
set(gca,'XTick',1:N_pars)
set(gca,'XTickLabel',parameter_names)
title('Parameters, FN')
set(gca, 'XTickLabelRotation', 90)
...
% Gray out non significant values
ytickangle(90)
