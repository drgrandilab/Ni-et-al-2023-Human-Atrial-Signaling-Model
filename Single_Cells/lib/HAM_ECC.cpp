...
void HAM_ECC::HAM_ECC_update_ODE(double t, signalling_para & para) {
	...
	double Ina_h_shift = (3.25 - 2 * 3.25 * 1 / (1 + exp(-(para.NaV_CKp - 0.12) / 0.1))); // h gate dependent on INa camKII
	...
	double GNa = (0.25 * para.kPKA_Ina_increase + 1) * (1 - 0.1 * AF) * SA_par[0] * 9; // [mS/uF] Courtemanche (adjusted for dV/dt max) MOD1
	...
	double GtoFast = 1.2 * 0.75 * para.Itof_Scale * 1.2 * (1 + (0.6 - 1) * para.kPKA_Itof) * SA_par[4] * (1 - 0.7 * AF) * 0.165; // 40% decrease w/ 100 nM ISO (Gonzales De La Fuente et al 2013)
	...
	I_kr = 1.1 * 0.9 * para.IKr_Scale * 0.35 * IKr_markov.IKr; // times 0.35 to match IKr magnitude in the original Grandi model.
	...
	// NEw IKur model, but not used in this model; // 16:37:09, Mon, 25-February-2019, By Haibo
	double CNZ_gkur = 1.05 * para.IKur_Scale * para.kCKII_Ikur * (1.0 + 1 * para.kPKA_Ikur) * (1 - 0.5 * AF) * 1.07 * 1.2 * 0.006398 * 0.85;
	...
	// I_kur = IKur;  //
	// ydot[44] = ydot[43] = ydot[42] = 0;


	//// I_ki: Time-Independent K Current
	double Gki = 0.75 * SA_par[8] * (1 + 1 * AF) * (2.1 * 0.0525) * sqrt(Ko / 5.4);
	...
	double gkp =  0.95 * SA_par[9] * 0.002;
	...
	double gk2p = 0.9 * para.IK2p_Scale * 0.95 * SA_par[10] * K2P_cond * 0.0050 * (1 + (SA_par[23] * 0.0145 / 0.005 - 1) * AF);
	...
	double gsk = 0.95 * para.ISK_Scale  * 0.85 * SA_par[12] * SK_cond * par_sk[0] * (1 + (SA_par[24] * par_sk[7] - 1) * AF);
	...
	// Li et al. An outwardly rectifying anionic background current in atrial myocytes from the human heart
	I_Clbk = para.IClb_Scale * 1.05 * 0.19e-3 * (y[39 - 1] - ecl) / (1 - 0.94 * exp(2.5e-4 * (y[39 - 1] - ecl))); 	
	...
	double GCaB = para.ICab_Scale * 6.0643e-4; // [uA/uF]
	...
	if (MOD_ind == 3) {
		IbarNCX =   1.1 * 1.05 * 1.2 * SA_par[19] * (1 + 0.8 * AF) * 3.15; // [uA/uF] MOD3
	}
...
void HAM_ECC::initialiser() {
	...
	V = y[38];   // for most up-to-date V, use the value from CVODE solver instead (for calculating dvdt_max) // 14:50:55, Tue, 29-June-2021, By Haibo
}
